...
The Predbat Table Card was originally created by [PaceMaker82](https://github.com/pacemaker82/PredBat-Table-Card).

I have used Claude to help add some additional functionality.
...
**Check out my new [compact power card](https://github.com/pacemaker82/Compact-Power-Card) if you are looking for a new power card. More of my [Predbat related enhancements here](https://smarter-home.blog/category/predbat/).**
...
| `show_predbat_version`   | NO    | Set to `true` if you want the card to show you the Predbat version. Click to upgrade when new version available! Appears at bottom of table |
| `show_tablecard_version`    | NO    | Set to `true` if you want the card to show you the card version. Click to upgrade when new version available! Appears at bottom of table |
...
| `reset_day_totals` | NO | `true` or `false`. Set to true if you want the totals column to reset at midnight |
...
| `hide_empty_columns` | NO | Set to `true` to automatically hide ANY column where there is no data for them in the plan, i.e. the column is empty of data. The columns will re-appear when Predbat has a plan for them (like when you plug your car back in) | 
